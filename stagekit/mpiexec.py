...
import asyncio
from typing import Callable, Dict, Tuple, Literal, List, Iterable, TYPE_CHECKING, cast
...
def _dispatch(lock: asyncio.Lock, nnodes: Fraction | int) -> bool:
    ...
    if mp:
        ...
        ntotal = config['job'].get('cpus_per_node') or _system.cpus_per_node
...
def splitargs(args: Iterable[str], nprocs: int) -> List[str]:
    """Split arguments to n processes."""
    # assign a chunk of arg_mpi to each processor
    args = sorted(args)
    output = []
    chunk = int(ceil(len(args) / nprocs))

    for i in range(nprocs - 1):
        output.append(args[i * chunk: (i + 1) * chunk])

    output.append(args[(nprocs - 1) * chunk:])

    return output


async def mpiexec(cmd: str | Callable,
                  nprocs: int, cpus_per_proc: int, gpus_per_proc: int | Tuple[Literal[1], int],
                  fname: str | None, args: list | tuple | None, mpiarg: list | tuple | None,
